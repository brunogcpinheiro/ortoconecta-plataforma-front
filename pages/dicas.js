...
class Dicas extends Component {
	...
	handleSubmit = async e => {
		...
		if (this.state.author && this.state.author_email && this.state.body) {
			await axios
				.post(
					`http://ortoconecta-plataforma-brunogcpinheiro.c9users.io:8080/tipsComments`,
					newComment,
				)
				.then(
					this.setState({
	...
	render () {
		const sorted = this.props.comments
				...
				<Main>
					<TipsWrapper>
						<Title data-aos="fade-right" data-aos-duration="1000">
							...
							<p>
								Uma área destinada à prática e conceitos modernos para você
								utilizar no dia a dia do seu consultório. Todo conhecimento
								agregado gera valor e faz com que você transmita a segurança
								necessária para cativar o seu paciente. <br />Navegue neste
								espaço e deixe seu comentário.
							</p>
						</Title>
						<TipContainer className="animated bounceIn delay-1s">
							{this.props.tips.map(tip => (
								<TipWrapper key={tip.id}>
									<VideoWrapper>
										<Iframe
											url={`https://www.youtube.com/embed/${tip.video_url}`}
											position="relative"
											width="100%"
											height="100%"
											display="initial"
							...
							<NewComment>
								...
								<form onSubmit={e => this.handleSubmit(e)}>
									...
									<input
										...
										placeholder="Email..."
										onChange={e =>
											this.setState({ author_email: e.target.value })}
							...
							{sorted.map(comment => (
								<Comment key={comment.id}>
									<div>
										...
										<small>
											{(dayjs.locale("pt-br"), dayjs(comment.date).fromNow())}
										</small>
										<p>{comment.body}</p>
									</div>
								</Comment>
